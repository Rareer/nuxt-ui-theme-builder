<template>
  <div class="ring ring-gray-200 rounded-lg p-4 space-y-3">
    ...
    <div class="space-y-3">
      ...
      <div v-if="local.type === 'color-reference'" class="mt-2">
        <UFormField>
          <USelect
        ...
        <div v-if="local.selectedColor" class="mt-3">
        ...
        <div v-if="local.value" class="mt-2 flex items-center gap-2">
          <div class="w-6 h-6 rounded-sm" :style="{ backgroundColor: getColorFromReference(local.value) }"></div>
      ...
      <div v-else class="mt-2">
        <UFormField>
          ...
          >
            <template #trailing>
              <div class="w-5 h-5 rounded-sm" :style="{ backgroundColor: local.value }"></div>
            </template>
          </UInput>
        </UFormField>
      </div>
...
const { shades, colorOptions, getColorShade, getColorFromReference, normalizeColorName } = useColorUtils()
