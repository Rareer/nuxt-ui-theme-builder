...
<template>
	...
	<div
		...
		class="flex-1 w-full"
	>
		<h2 class="text-lg font-semibold my-8">
			Preview Options
		</h2>
		<div class="space-y-4">
			<div class="flex flex-wrap items-center gap-6">
				<UFormField
					v-for="p in booleanProps"
					:key="p.name"
					:label="p.name"
				>
			...
			<div class="flex flex-wrap items-center gap-6">
				<UFormField
					v-for="p in optionProps"
					:key="p.name"
					:label="p.name"
				>
					<USelect
						v-model="bound[p.name]"
						class="block w-full"
	...
	<UTabs
		v-if="configPropNames.length && uiSlots.length"
		color="neutral"
		:items="configPropNames.map(n => ({ label: n }))"
		class="my-6"
	>
		...
		<template #content="{ item }">
			<div class="space-y-6">
				<div
					v-for="opt in (optionsByProp[(item as any).label] || [])"
					:key="opt"
					class="space-y-3"
				>
					<!-- @vue-ignore -->
