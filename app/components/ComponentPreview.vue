	...
	<div class="flex lg:flex-row flex-col gap-6">
		<div class="flex-1">
			...
			<div class="space-y-6 w-full grid grid-cols-2 gap-4">
				...
				>
					...
					>
						<template
							v-if="config?.hasHeader"
							#header
						>
							<span class="h-8">Header</span>
						</template>
						<span
							v-if="config?.hasContent"
							class="h-32"
						>Content</span>
						<template
							v-if="config?.hasFooter"
							#footer
						>
		...
		<div class="flex-1 lg:pl-6 lg:border-l border-gray-200 dark:border-gray-800">
			<div class="flex grow gap-4">
				...
				<UFormField
					v-if="config?.hasTrailingIcon && showIcon"
					label="Trailing Icon"
				>
				...
				<template #variants="{ item }">
					...
					>
						<UFormField :label="variant">
							<Combobox
								:model-value="getKeyClasses('variants', variant)"
								@update:model-value="val => setKeyClasses('variants', variant, val)"
							/>
							<UButton
								size="xs"
								color="neutral"
						...
						<div
							v-if="isExpanded('variants', variant)"
							class="pl-4 border-l-2 border-gray-200 mt-2 mb-4 space-y-4"
						>
							<div
								v-for="uiProp in uiProperties"
								v-show="uiProp !== uiRootName"
								:key="uiProp"
							>
								<UFormField :label="uiProp">
									<Combobox
										:model-value="getUiSlotClasses('variants', variant, uiProp)"
										@update:model-value="val => setUiSlotClasses('variants', variant, uiProp, val)"
									/>
								</UFormField>
							</div>
						</div>
				...
				<template #colors="{ item }">
					...
					>
						<UFormField :label="color.value">
							<Combobox
								:model-value="getKeyClasses('colors', color.value)"
								@update:model-value="val => setKeyClasses('colors', color.value, val)"
							/>
							<UButton
								size="xs"
								color="neutral"
						...
						<div
							v-if="isExpanded('colors', color.value)"
							class="pl-4 border-l-2 border-gray-200 mt-2 mb-4 space-y-4"
						>
							<div
								v-for="uiProp in uiProperties"
							...
							>
								<UFormField :label="uiProp">
									<Combobox
										:model-value="getUiSlotClasses('colors', color.value, uiProp)"
										@update:model-value="val => setUiSlotClasses('colors', color.value, uiProp, val)"
									/>
								</UFormField>
							</div>
						</div>
				...
				<template #sizes="{ item }">
					...
					>
						<UFormField :label="size">
							<Combobox
								:model-value="getKeyClasses('sizes', size)"
								@update:model-value="val => setKeyClasses('sizes', size, val)"
							/>
							<UButton
								size="xs"
								color="neutral"
						...
						>
							...
							>
								<UFormField :label="uiProp">
									<Combobox
										:model-value="getUiSlotClasses('sizes', size, uiProp)"
										@update:model-value="val => setUiSlotClasses('sizes', size, uiProp, val)"
									/>
								</UFormField>
							</div>
						</div>
